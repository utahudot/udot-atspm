DO $$
BEGIN
   IF NOT EXISTS (SELECT FROM pg_database WHERE datname = 'ATSPM-Aggregation') THEN
      PERFORM dblink_exec('dbname=postgres user=admin password=Avenue.2006', 'CREATE DATABASE ATSPM-Aggregation');
   END IF;
END
$$;
