...
export const getGetEventLogDaysWithDataFromLocationIdentifierAndDataTypeQueryKey = (locationIdentifier: string,
    dataType: 'IndianaEvent' | 'PedestrianCounter' | 'SpeedEvent',
    params?: GetEventLogDaysWithDataFromLocationIdentifierAndDataTypeParams,) => {
    return [`/api/v1/EventLog/GetDaysWithData/${locationIdentifier}/${dataType}`, ...(params ? [params]: [])] as const;
    }
...
const {query: queryOptions} = options ?? {};
  ...
  const queryKey =  queryOptions?.queryKey ?? getGetEventLogDaysWithDataFromLocationIdentifierAndDataTypeQueryKey(locationIdentifier,dataType,params);
   ...
   return  { queryKey, queryFn, enabled: !!(locationIdentifier && dataType), ...queryOptions} as UseQueryOptions<Awaited<ReturnType<typeof getEventLogDaysWithDataFromLocationIdentifierAndDataType>>, TError, TData> & { queryKey: QueryKey }
}
