...
// for WebUI - speedCompliance.transformer.ts
//
...
// #endregion
import { SpeedComplianceDto } from '@/api/speedManagement/aTSPMSpeedManagementApi.schemas'
import {
  createDataZoom,
  createGrid,
  createLegend,
...
export default function transformSpeedComplianceData(
  ...
  const dataZoom = addDataZoom ? createDataZoom() : undefined
...
function mergeSeriesData(
  ...
  response.forEach((segment, index) => {
    ...
    tableData.speedLimit.push(
      finalSpeedLimit !== undefined ? finalSpeedLimit : null
    )
