...
// for Application - Utah.Udot.Atspm.Common/ControllerLoggerProgress.cs
// 
...
namespace Utah.Udot.Atspm.Common
{
    public abstract class ControllerLoggerProgress<T>
    {
        ...
        {
            if (HasException)
                return $"{GetType().Name}: {Exception?.GetType().Name} {Current}/{Total} Successful {IsSuccessful}";

            return $"{GetType().Name}: {_item?.GetType().Name} {Current}/{Total} Successful {IsSuccessful}";
        }
    }
