...
namespace ATSPM.DataApi.Controllers
{
    ...
    public class TestEventController : ControllerBase
    {
        ...
        /// <inheritdoc/>
        public TestEventController(IEventLogRepository repository, ILogger<TestEventController> log)
        {
        ...
        public IActionResult GetLocationEventsBetweenDates(string locationIdentifier, DateOnly date)
        {
            if (date == DateOnly.MinValue || date == DateOnly.MinValue)
                return BadRequest("Invalid date");

            var result = _repository.GetEvents(locationIdentifier, date);
            //var result = _repository.GetList().Where(w => w.LocationIdentifier == locationIdentifier && w.ArchiveDate == date).ToList();
