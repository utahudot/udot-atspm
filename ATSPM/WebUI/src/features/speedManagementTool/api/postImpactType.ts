...
const postImpactType = async (impactData: { name: string; description: string }): Promise<ImpactType> => {
    console.log(`switch for ${SPEED_URL} once cloud works`)
  
    // Correcting the axios.post call
    const response = await axios.post<ImpactType>(`${localhostURL}api/ImpactType`, impactData, {
      headers: {
  ...
  export const usePostImpactType = (config?: QueryConfig<typeof postImpactType>) => {
    return useMutation({
