...
public class CreateArchiveLogForTestSite : Command
{
    private readonly ILogger<CreateArchiveLogForTestSite> logger;
    private readonly IServiceProvider serviceProvider;
    private readonly ILocationRepository locationRepository;

    public CreateArchiveLogForTestSite(
        ILogger<CreateArchiveLogForTestSite> logger,
        IServiceProvider serviceProvider,
        ILocationRepository locationRepository) : base("create-test", "Create Archive Log command")
    {
    ...
    private static void GetLogs(DateOnly dateToRetrieve, SqlConnection sourceConnection, List<CompressedEventLogs<IndianaEvent>> archiveLogs, Location? location, IConfiguration config)
    {
        ...
        using (SqlCommand selectCommand = new SqlCommand(selectQuery, sourceConnection))
        {
            ...
            {
                var device = location.Devices.FirstOrDefault(d => d.DeviceType == ATSPM.Data.Enums.DeviceTypes.SignalController);
                if (device != null)
                {
