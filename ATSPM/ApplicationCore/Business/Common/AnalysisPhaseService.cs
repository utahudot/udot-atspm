...
namespace ATSPM.Application.Business.Common
{
    ...
    public class AnalysisPhaseService
    {
            ...
            )
        {
            ...
            var cycleEventCodes = new List<IndianaEnumerations> { IndianaEnumerations.PhaseBeginGreen, IndianaEnumerations.PhaseBeginYellowChange, IndianaEnumerations.PhaseEndRedClearance };
            ...
            {
                analysisPhaseData.TerminationEvents = cleanTerminationEventsForPhase.Where(t => t.EventParam == phasenumber && (t.EventCode == IndianaEnumerations.PhaseGapOut || t.EventCode == IndianaEnumerations.PhaseMaxOut || t.EventCode == IndianaEnumerations.PhaseForceOff)).ToList();
            }
            else
            {
                analysisPhaseData.TerminationEvents = new List<IndianaEvent>();
            }
            analysisPhaseData.ConsecutiveGapOuts = FindConsecutiveEvents(analysisPhaseData.TerminationEvents, IndianaEnumerations.PhaseGapOut, consecutiveCount) ?? new List<IndianaEvent>();
            analysisPhaseData.ConsecutiveMaxOut = FindConsecutiveEvents(analysisPhaseData.TerminationEvents, IndianaEnumerations.PhaseMaxOut, consecutiveCount) ?? new List<IndianaEvent>();
            analysisPhaseData.ConsecutiveForceOff = FindConsecutiveEvents(analysisPhaseData.TerminationEvents, IndianaEnumerations.PhaseForceOff, consecutiveCount) ?? new List<IndianaEvent>();
            ...
            analysisPhaseData.PercentMaxOuts = FindPercentageConsecutiveEvents(analysisPhaseData.TerminationEvents, IndianaEnumerations.PhaseMaxOut);
            analysisPhaseData.PercentForceOffs = FindPercentageConsecutiveEvents(analysisPhaseData.TerminationEvents, IndianaEnumerations.PhaseForceOff);
            ...
            int phasenumber)
        {
            ...
            for (int i = 0; i < sortedEvents.Count - 1; i++)
            {
                ...
                {
                    if (event1.EventCode == IndianaEnumerations.PhaseGreenTermination)
                        duplicateList.Add(event1);
                    if (event2.EventCode == IndianaEnumerations.PhaseGreenTermination)
            ...
            int phasenumber)
        {
            var events = (from row in terminationeventstable
                          where row.EventParam == phasenumber && (row.EventCode == IndianaEnumerations.PedestrianBeginWalk || row.EventCode == IndianaEnumerations.PedestrianBeginSolidDontWalk)
        ...
        private List<IndianaEvent> FindConsecutiveEvents(List<IndianaEvent> terminationEvents,
            IndianaEnumerations eventtype, int consecutiveCount)
        {
        ...
        {
            return terminationEvents.Where(t => t.EventCode == IndianaEnumerations.PhaseGreenTermination && t.EventParam == phaseNumber).ToList();
        }


        private double FindPercentageConsecutiveEvents(List<IndianaEvent> terminationEvents, IndianaEnumerations eventtype)
        {
            double percentile = 0;
            double total = terminationEvents.Count(t => t.EventCode != IndianaEnumerations.PhaseGreenTermination);
