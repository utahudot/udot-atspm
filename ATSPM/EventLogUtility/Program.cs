...
//var detector = new Detector() { DetChannel = 2, DistanceFromStopBar = 340, LatencyCorrection = 1.2, Approach = new Approach() { Mph = 45 } };

//var redCycles = await createRedToRedCycles.ExecuteAsync(list);
//var vehicles = await identifyandAdjustVehicleActivations.ExecuteAsync(Tuple.Create<Detector, IEnumerable<ControllerEventLog>>(detector, list));

//foreach (var r in redCycles.SelectMany(s => s))
//{
//    Console.WriteLine($"{r.GetHashCode()}-{r.VehicleEvents.Count()}");
//}
...
var broadcast = new BroadcastBlock<IEnumerable<ControllerEventLog>>(null);
//TODO: add the broadcast filters here
var GetDetectorEvents = new TransformManyBlock<IEnumerable<ControllerEventLog>, Tuple<Detector, IEnumerable<ControllerEventLog>>>(t =>
{
    var result = t.Where(l => l.EventCode == (int)DataLoggerEnum.DetectorOn)
...
var GroupSignal = new TransformManyBlock<IEnumerable<ControllerEventLog>, IEnumerable<ControllerEventLog>>(t =>
{
    return t.GroupBy(g => g.SignalId);
});
...
var mergeCalculateDelayValues = new JoinBlock<IEnumerable<Vehicle>, IEnumerable<RedToRedCycle>>();


var test = new ActionBlock<Tuple<IEnumerable<Vehicle>, IEnumerable<RedToRedCycle>>>(a =>
{
    var vehicles = new List<Vehicle>();


    foreach (var v in a.Item1)
    {
        v.RedToRedCycle = a.Item2.FirstOrDefault(w => w.SignalId == v.SignalId && v.TimeStamp >= w.StartTime && v.TimeStamp <= w.EndTime);
    ...
    //Console.WriteLine($"count {vehicles.Count()}");

    //Console.WriteLine($"avg {vehicles.Where(w => w.Delay > 0).Average(a => a.Delay)}");

    var result = vehicles.GroupBy(g => g.SignalId).Select(s => new ApproachDelayResult() { Start = a.Item2.Min(m => m.StartTime), End = a.Item2.Max(m => m.EndTime), SignalId = s.Key, Vehicles = s.ToList() });
...
var test1 = new ActionBlock<IEnumerable<RedToRedCycle>>(a =>
{
    foreach (var i in a)
    {
        Console.WriteLine($"redtored {i}");
    }
});

var test2 = new ActionBlock<IEnumerable<Vehicle>>(a =>
{
    foreach (var i in a)
    {
...
//var test = new ActionBlock<IEnumerable<RedToRedCycle>>(a =>
//{
//    foreach (var i in a)
//    {
//    Console.WriteLine($"{i}");
...
GroupSignal.LinkTo(broadcast, new DataflowLinkOptions() { PropagateCompletion = true });
broadcast.LinkTo(CreateRedToRedCycles, new DataflowLinkOptions() { PropagateCompletion = true });
broadcast.LinkTo(GetDetectorEvents, new DataflowLinkOptions() { PropagateCompletion = true });
GetDetectorEvents.LinkTo(IdentifyandAdjustVehicleActivations, new DataflowLinkOptions() { PropagateCompletion = true });

//CreateRedToRedCycles.LinkTo(test1, new DataflowLinkOptions() { PropagateCompletion = true });
//IdentifyandAdjustVehicleActivations.LinkTo(test2, new DataflowLinkOptions() { PropagateCompletion = true });
...
mergeCalculateDelayValues.LinkTo(test, new DataflowLinkOptions() { PropagateCompletion = true });



GroupSignal.Post(list);
GroupSignal.Complete();
...
//CreateRedToRedCycles.LinkTo(test, new DataflowLinkOptions() { PropagateCompletion = true });

//CreateRedToRedCycles.Post(list);

//CreateRedToRedCycles.Complete();

//while (await CreateRedToRedCycles.OutputAvailableAsync())
//{
//    foreach (var cycleGroup in await CreateRedToRedCycles.ReceiveAsync())
//    {
//        Console.WriteLine($"{cycleGroup}");
//    }
//}
