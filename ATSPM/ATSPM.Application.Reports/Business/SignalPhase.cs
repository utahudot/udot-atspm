...
namespace ATSPM.Application.Reports.Business
{
    public class SignalPhase
    {
        ...
        private void GetSignalPhaseData(IControllerEventLogRepository controllerEventLogRepository)
        {
            GetDetectorEvents(_metricTypeId, controllerEventLogRepository);
        ...
        private void GetDetectorEvents(int metricTypeId, IControllerEventLogRepository controllerEventLogRepository)
        {
            DetectorEvents = new List<ControllerEventLog>();
            var detectorsForMetric = Approach.GetDetectorsForMetricType(metricTypeId);
            foreach (var d in detectorsForMetric)
                DetectorEvents.AddRange(controllerEventLogRepository.GetEventsByEventCodesParam(
                    Approach.SignalId,
                    StartDate,
                    EndDate,
                    new List<int> { 82 },
